...
- mkpms：根构建 `ctest` 统一覆盖 16 套件 — `ghostmem_abi_test`（17）、`ghostmem_client_overlap_test`（14）、`ghostmem_args_test`（8）、`ghostmem_pgtable_harness`（37）、`ghostmem_core_harness`（26）、`ghostlinker_test`（21）、`ghostlinker_cli_run`（1）、`ghostmem_client_e2e_test`（7+1 skip）、`wxshadow_abi_test`（8）、`wxshadow_client_e2e_test`（5）、`wxshadow_pte_state_test`（15）、`wxshadow_brk_esr_test`（15）、`wxshadow_tlb_test`（6）、`wxshadow_next_task_test`（4）、`wxshadow_addr_test`（11）、`vma_offset_test`（7）
