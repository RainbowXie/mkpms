...
- mkpms：根构建 `ctest` 统一覆盖 6 套件 — `ghostmem_abi_test`（12）、`ghostmem_client_overlap_test`（8）、`ghostmem_args_test`（8）、`ghostmem_pgtable_harness`（26）、`ghostmem_core_harness`（15）、`ghostlinker_test`（12）
