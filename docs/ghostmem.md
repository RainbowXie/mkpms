...
| `PR_GHOSTMEM_WRITE (0x47474d04)` | `prctl(opt, pid, va, buf, len)` | 跨进程写幽灵内存（内核 PTE，无 ptrace 依赖）；限 64 页 |
| `PR_GHOSTMEM_READ (0x47474d05)` | `prctl(opt, pid, va, buf, len)` | 跨进程读幽灵内存（内核 PTE） |
